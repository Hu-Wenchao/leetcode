...
class Solution(object):
    ...
    def exist(self, board, word, i, j):
        if board[i][j] == word[0]:
            ...
            if i > 0 and self.exist(board, word[1:], i-1, j):
                return True
            if i < len(board)-1 and self.exist(board, word[1:], i+1, j):
                return True
            if j > 0 and self.exist(board, word[1:], i, j-1):
                return True
            if j < len(board[0])-1 and self.exist(board, word[1:], i, j+1):
                return True
